...
if (!empty($_POST['mode'])) {
    ...
    case "regist":
        ...
        if (!empty($_POST['wewlad'])) {
            $mitsuba->common->showMsg($lang['img/error'], "Your post looks automated. To prevent spam and other malicous actions, your post was rejected.");
      ...
      if ((!$ignoresCaptcha) && (intval($bdata['captcha']) == 1) && (!($notExpired))) {
        ...
        if (!$mitsuba->admin->checkPermission("post.ignorespamfilter")) {
            $mitsuba->board->checkSpam(isset($_POST['name']), $_POST['board']);
            $mitsuba->board->checkSpam(isset($_POST['email']), $_POST['board']);
            $mitsuba->board->checkSpam(isset($_POST['sub']), $_POST['board']);
            $mitsuba->board->checkSpam(isset($_POST['com']), $_POST['board']);
        }
        ...
        if ($mod >= 1) {
            ...
            if ((!empty($_POST['capcode'])) && ($_POST['capcode'] == 1) && ($mitsuba->admin->checkPermission("post.capcode"))) {
                ...
                }elseif(($mitsuba->admin->canBoard("$"))) {
                  echo("\u{1696}");
                    $cc_text = "\u{1696} Developer";
                    $cc_style = "color: #4B088A";
                }else{
       ...
       if ((substr($filename ?? '', 0, 6) != "embed:") && ($gen_thumb == 1)) {
            ...
            if (!empty($_FILES['upfile']['tmp_name'])) {
                ...
                if ($resto != 0) {
                    ...
                    if ((empty($returned['width'])) || (empty($returned['height']))) {
                        ...
                        $mitsuba->common->showMsg($lang['img/error'], $lang['img/no_thumb']."1");
                    ...
                    if ((empty($returned['width'])) || (empty($returned['height']))) {
                        ...
                        $mitsuba->common->showMsg($lang['img/error'], $lang['img/no_thumb']."2");
        ...
        if ($is == - 16) {
            ...
            if ($bdata['type'] == "overboard") {
                $mitsuba->caching->rebuildBoardCache($_POST['board']);
            }
        ...
        if (!empty($_POST['delete'])) {
            ...
            foreach ($_POST as $key => $value) {
                ...
                if ($value == "delete") {
                    ...
                    if ($done == - 1) {
                        $mitsuba->common->showMsg($lang['img/error'], sprintf($lang["img/post_bad_password"], "/". $keys[1] . "/" . $keys[2]));
                        exit;
                    } elseif ($done == - 2) {
                        $mitsuba->common->showMsg($lang['img/error'], sprintf($lang["img/post_not_found"], "/" . $keys[1] . "/" . $keys[2]));
                        exit;
                    } elseif ($done == - 3) {
                        $mitsuba->common->showMsg($lang['img/error'], sprintf($lang["img/post_no_image"], "/" . $keys[1] . "/" . $keys[2]));
                        exit;
                    } elseif ($done == - 4) {
                        $mitsuba->common->showMsg($lang['img/error'], sprintf($lang["img/post_wait_more"], "/" . $keys[1] . "/" . $keys[2]));
                    } elseif ($done == 1) {
                        $mitsuba->common->showMsg(sprintf($lang["img/post_deleted_image"], "/" . $keys[1] . "/" . $keys[2]), sprintf($lang["img/post_deleted_image"], "/" . $keys[1] . "/" . $keys[2]));
                    } elseif ($done == 2) {
                        $mitsuba->common->showMsg(sprintf($lang["img/post_deleted"], "/" . $keys[1] . "/" . $keys[2]), sprintf($lang["img/post_deleted"], "/" . $keys[1] . "/" . $keys[2]));
                    }
