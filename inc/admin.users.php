    ...
    function updateUser($identifier, $username, $password, $group, $boards)
    {
        ...
        if ($user->num_rows == 1) {
            ...
            if (!empty($password)) {
                ...
                $passwordDB = ", password='" . hash("sha512", $password . $userdata['salt']) . "'";

            }

            $boards = $this->conn->real_escape_string($boards);

            $this->conn->query("UPDATE users SET username='" . $username . "'" . $passwordDB . ", `group`=" . $group . ", boards='" . $boards . "' WHERE id=" . $identifier);
