...
<?php echo $lang['mod/ip']; ?>: <strong><?php echo mb_strimwidth(crypt($IPAddress, $securetrip_salt), 0, 10); ?></strong><br />
    ...
    exit;
        ...
        if (!$canBan) {
            ...
            if($mitsuba->admin->canBoard("%")) {
                ...
                // Get the IP address from $_POST['post'] and $_POST['board']...
                $gIPAddr = $conn->query("SELECT * FROM posts WHERE id=".$_POST['post']." AND board='".$_POST['board']."'");
                if($gIPAddr->num_rows == 1) {
                    $gIPPostInfo = $gIPAddr->fetch_assoc();
                    $PostIPAddr = $gIPPostInfo['ip'];
                }else{

                }
                // ... And run the addBan function.
                $result = $mitsuba->admin->bans->addBan($PostIPAddr, $_POST['reason'], $_POST['note'], $_POST['expires'], $boards, $_POST['appeal']);
            }
            if ($result != -2) {
                if ((!empty($_POST['delete'])) && ($_POST['delete']=="1")) {
