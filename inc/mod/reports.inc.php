...

{
    ...
    echo "<td class='text-center text-nowrap'>[ <a href='?/reports&cl=1&id=".$row['id']."' title='Clear Report'>C</a> ] [ <a href='?/bans/add&b=".$row['board']."&p=".$row['reported_post']."' title='Ban Poster'>B</a> ";

    if ($mitsuba->admin->checkPermission("reports.clear.multiple")) {

        echo "/ <a href='?/bans/add&b=".$row['board']."&p=".$row['reported_post']."&d=1' title='Ban and Delete'>&</a> / <a href='?/delete_post&b=".$row['board']."&p=".$row['reported_post']."' title='Delete Post'>D</a> / <a href='?/delete_post&b=".$row['board']."&p=".$row['reported_post']."&f=1' title='Delete File'>F</a> ] <br />";

        echo "[ <a href='?/info&ip=".$pdata['ip']."' title='IP Notes'>I</a> ] <br />";
        if($mitsuba->admin->canBoard("%")) {
            //Eradicate Doppelgangers and Eradicate Doppelgangers with IP address
            echo "[ <a href='?/reports&m=wtr&i=".$row['id']."' title='Delete all Duplicates with the same reason'>ED</a> / <a href='?/reports&m=ip&i=".$row['id']."'title='Delete all Duplicates with the same reporter IP'>EDIP</a> ]";
        }
        echo "</td>";
...

{
    ...
    if ($mitsuba->admin->checkPermission("reports.clear.multiple")) {
        ...
        echo "[ <a href='?/info&ip=".$pdata['ip']."' title='Poster IP Notes'>I</a> ] <br />";

        echo "[ <a href='?/reports&m=wtr&i=".$row['id']."'>Clear all reports with same reason</a> / <a href='?/reports&m=ip&i=".$row['id']."'>D_WTIP</a> ]";
