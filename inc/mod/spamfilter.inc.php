...
if ((!empty($_POST['mode'])) && ($_POST['mode'] == "add")) {
    ...
    if (empty($_POST['search'])) { echo "<b style='color: red;'>".$lang['mod/fill_all_fields']."</b>"; 
    } else { $search = $_POST['search']; $continue = 1; 
    }

    if (empty($_POST['reason'])) { echo "<b style='color: red;'>".$lang['mod/fill_all_fields']."</b>"; 
    } else { $reason = $_POST['reason']; $continue = 1; 
    }
...

{
    ...
    echo "<td class='text-center'>"; if ($row['regex'] == 1) { echo($lang['mod/yes']);
    } else {echo($lang['mod/no']);
    } echo "</td>";
...
<?php echo $lang['mod/regex'] ?>: <input type="checkbox" name="regex" <?php if($regex == 1) { echo "checked"; 
}?>"/><br />
