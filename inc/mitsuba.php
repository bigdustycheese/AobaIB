...
class Admin
{
    ...
    function appendToPost($board, $postid, $text)
    {
        ...
        if (is_numeric($postid)) {
            ...
            if ($post->num_rows == 1) {
                ...
                $newText = $this->conn->real_escape_string($pdata['comment']) . "\n\n<+" . $text . "+>";

                $this->conn->query("UPDATE posts SET comment='" . $newText . "', raw=2 WHERE id=" . $postid . " AND board='" . $board . "'");
    ...
    function canBoard($board)
    {
        ...
        if ($_SESSION['boards'] != "%" && $this->conn->query("SELECT * FROM `users` WHERE `users`.`boards` = '%' AND `users`.`username` = '".$_SESSION['username']."'")) {
