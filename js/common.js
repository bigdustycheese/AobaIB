...
//!localStorage.o_pinned && (localStorage.o_pinned = 1);  // tymczasowo
...
if (localStorage.getItem('o_night_css') == 1) {
	...
	if (CurrHour >= '18'|| CurrHour <= '5'){
		...
		linkTheme.onload = function() {
			...
			localStorage.setItem('night_theme_css', css);
...
if (localStorage.getItem('dont_worry_about_it') === "1" && !inThread && !inCatalog){

		var path = '/css/tcv.css';

		//create it inline temporarily while page is loading from localstorage, prevents bright flash
		var css = localStorage.getItem('tcv_css');
		if (css != null) {
			var inlineTheme = document.createElement('style');
			document.head.appendChild(inlineTheme);
			inlineTheme.innerHTML = css;
		...
		linkTheme.onload = function() {
			...
			localStorage.setItem('tcv_css', css);
			//remove the inline style afterwards, not needed anymore
 			if (inlineTheme) {
				inlineTheme.remove();
			}
...
/*Keyboard shortcut for submitting form */
...
//local code for /comf/
if (window.location.pathname.split('/')[1] == "comf") {
...
$(document).ready(function () {
	...
	if (typeof Cookies.get("in_mod") !== "undefined")
	{
		adminStuff("body");
	}
...
{
	if (typeof Cookies.get("mitsuba_name") !== "undefined")
	{
		$(parent).find("input[name='name']").val(Cookies.get("mitsuba_name"));
	}

	if (typeof Cookies.get("mitsuba_email") !== "undefined")
	{
		$(parent).find("input[name='email']").val(Cookies.get("mitsuba_email"));
	}

	if (typeof Cookies.get("mitsuba_fakeid") !== "undefined")
	{
		$(parent).find("input[name='fake_id']").val(Cookies.get("mitsuba_fakeid"));
	}

	if (typeof Cookies.get("password") !== "undefined")
	{
		$(parent).find("input[name='pwd']").val(Cookies.get("password"));
	}
}
...
function updateThread(isAuto){
	...
	var threadID = $(".thread:first");
	var url = window.location.href.split(/#/)[0];

	var request = new XMLHttpRequest();
	request.open('GET', "/" + resID + ".json"+"?random="+Math.floor(Math.random() * 900000), true);
	request.onload = function(){
		if(request.status >=200 && request.status <= 400){
			var lastPost = $(".replyContainer").length;
			var html = request.responseText;
		  var nodes = JSON.parse(html);
			var apiPostNum = Object.keys(nodes.posts).length-1;
		  var newposts = 0;

			var elements = document.querySelectorAll(".postContainer");
			if (apiPostNum > lastPost) {
				for (const post of nodes.posts){
					const el = document.getElementById('p'+post.no);
					if (el){
						// update el if anything has changed - might not be possible for them to change?
					} else {
						// insert new post html after document.getElementById('p'+(post.no-1));
						var postTime = new Date(post.time);
						var week = new Array('Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat');
						var timeString = postTime.getDate() + "/" + (postTime.getMonth() + 1) + "/" + postTime.getFullYear() + "("+ week[postTime.getDay()] + ")" + postTime.getHours() + ":" + postTime.getMinutes()+":" + postTime.getSeconds();
						$(threadID).append('<div class="postContainer replyContainer" id="pc'+( post.no - 1 )+'">'+
					'<div class="sideArrows" id="sa'+( post.no - 1 )+'">&gt;&gt;</div>'
					+'<div id="p'+( post.no - 1 )+'" class="post reply">'
					+'<div class="postInfo" id="pi'+( post.no - 1 )+'">'
					+'<input type="checkbox" name="#" value="delete"> <span class="subject">'+( String(post.sub) )+'</span>'
					+'<span class="nameBlock"><span class="name">'+( String(post.name) )+'</span></span>'
					+'<span class="dateTime">'+( timeString )+'</span>'
					+'<span class="postNum"><a href="#" title="Highlight this post">No.</a><a href="#" class="quotePost" id="z'+( post.no - 1 )+'" title="Quote this post">'+ (apiPostNum + 1 )+'</a> &nbsp;</span>'
					+'<blockquote class="postMessage" id="m22">'+( String(post.com) )+'<br><div class="backlink" id="bl22"></div></blockquote>'
					+''
					+'</div>'
					+'</div>'
					+'</div>');
						$("#p"+ ( post.no - 1 )).addClass("postnew");
					}
				}
			}
		    ...
		    if (postID > lastRead)
		    {
		      ...
		      newposts++;
		      totalUnread++;
		      $(threadID).append('<div class="postContainer replyContainer" id="pc'+postID+'">'+$(this).html()+'</div>');
		      $("#p"+postID).addClass("postnew");
		    }
		    $("#p"+postID).removeClass("postdeleted");
		  });
			addQuotelinks();
			addBacklinks(threadID);
			if (localStorage.getItem("o_preview") == 1){
		    addPostpreview(threadID);
		  }

			if (localStorage.getItem("o_imgexpand") == 1){
		    addImgExpand(threadID);
		  }
...
function addStylechanger(){
	...
	$("link[rel='alternate stylesheet']").each(function () {
		var selected = "";
		if (typeof Cookies.get("mitsuba_style") !== "undefined")
		{
			if (absolutizeURI(window.location.href, $(this).attr("href")) == Cookies.get("mitsuba_style"))
			{
				selected = " selected";
			}
		}
	...
	$("#stylechanger").change(function (e) {
		...
		Cookies.set("mitsuba_style", absolutizeURI(window.location.href, e.target.options[e.target.selectedIndex].value), {expires: 31,sameSite: 'Lax',path: '/'});
	});
...
function addThreadExpander(parent)
{
	...
	$(parent).find(".thread").each(function () {
		$('<a href="javascript:;" class="expander" id="e'+$(this).attr("id")+'"><i class="fa fa-expand" title="Expand Thread"></i></a>').insertAfter($("div#"+$(this).attr("id")+" > span.summary")).click(function () {
			...
			var href = absolutizeURI(window.location.href, $(threadID).find(".replylink").attr("href"));
			$.ajax({
			type: 'get',
			url: href,
			success: function(data, textStatus, xhr){
				var html = xhr.responseText;
				var nodes = $.parseHTML( html );
				$(threadID).fadeOut(100, function()
					{
						$(threadID).fadeIn(200);
					});

				$(threadID).html($(threadID, nodes).html());

				$('<a href="javascript:;" class="hider" id="ht'+threadID.substr(2)+'"><i class="fa fa-eye-slash" title="Hide Thread"></i></a>').appendTo($(threadID+" div.op div.postInfo")).click(function () {
					var id = $(this).attr("id").substr(2);
					thread_toggle(id);
				});
				$('<span> &nbsp; [<a href="'+href+'" class="replylink">Reply</a>] </span>').insertAfter($(threadID+" div.op span.postNum"));
				$(threadID).find("a").each( function () { if ($(this).attr("href") !== null) { $(this).attr("href", absolutizeURI(href, $(this).attr("href"))); } } );
				$(threadID).find("img").each( function () { $(this).attr("src", absolutizeURI(href, $(this).attr("src")));  } );
				...
				if (localStorage.getItem("o_watched") == 1)
				{
					addWatchButton(threadID);
				}
				if (localStorage.getItem("o_fastreply") == 1)
				{
					addFastReply(threadID, 1);
				}
				if (typeof Cookies.get("in_mod") !== "undefined")
				{
					adminStuff(threadID);
...
{
	var permissions_raw = Cookies.get("in_mod");
			...
			if (permissions['post.viewip']==1)
			{
				$.ajax({
					...
					success: function(data, textStatus, xhr){
						...
						{
							opIp = json.ip;
							$(el).children(".nameBlock").after(' <span class="posterIp">(<a href="'+api_url+'/info&ip='+json.ip+'" title="IP notes">'+json.ip+'</a>)</span> <b style="color: red;" title="This user is the OP">[OP]</b>');
						}
					}
				});
			}
				...
				$.ajax({
					...
					success: function(data, textStatus, xhr){
						...
						if (json.error != 404)
						{
							...
							$(el).children(".nameBlock").after(' <span class="posterIp">(<a href="'+api_url+'/info&ip='+json.ip+'" title="IP notes">'+json.ip+'</a>)</span>'+op);
						}
					}
				});
			}
